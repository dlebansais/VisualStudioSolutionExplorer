...
set RESULTFILEPATH=".\bin\%PLATFORM%\%CONFIGURATION%\%FRAMEWORK%\%RESULTFILENAME%"
set MODULEFILEPATH=".\bin\%PLATFORM%\%CONFIGURATION%\%FRAMEWORK%\%TESTPROJECTNAME%.dll"
...
rem dotnet build ./Test/%TESTPROJECTNAME% /p:Platform=%PLATFORM% -c %CONFIGURATION% -f %FRAMEWORK%

rem Execute tests within OpenCover.
pushd Test\%TESTPROJECTNAME%

if exist %RESULTFILEPATH% del %RESULTFILEPATH%
"%BASE_DIR%\%OPENCOVER_EXE%" -register:user -target:"C:\Program Files\dotnet\dotnet.exe" -targetargs:"test --output:detailed --test-modules %MODULEFILEPATH%" -output:%RESULTFILEPATH% -mergeoutput
if not exist %RESULTFILEPATH% goto error_no_test_result

"%BASE_DIR%\%CODECOV_UPLOADER_EXE%" -f %RESULTFILEPATH% -t %TOKEN%
"%BASE_DIR%\%REPORTGENERATOR_EXE%" -reports:%RESULTFILEPATH% -targetdir:%BASE_DIR%\CoverageReports "-assemblyfilters:+%PROJECTNAME%;+%TESTPROJECTNAME%" "-filefilters:-*.g.cs;-*Microsoft.NET.Test.Sdk.Program.cs"
popd
goto end
